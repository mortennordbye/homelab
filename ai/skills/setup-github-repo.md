...
|---|---|---|
| Invisible security: Dependabot alerts + security updates, secret scanning, private vulnerability reporting, CodeQL, dependency review, Trivy, read-only Actions token | yes | yes |
...
- **`README.md`** — use the template below. Fill `<owner>/<repo>` from introspection and
  only include badges for workflows that actually get created.
...
### A brief, punchy subtitle explaining what this is.

[![CI](https://img.shields.io/github/actions/workflow/status/<owner>/<repo>/ci.yml?style=flat-square)](https://github.com/<owner>/<repo>/actions) [![License](https://img.shields.io/github/license/<owner>/<repo>?style=flat-square)](LICENSE)

[A short paragraph: why does this exist, what problem does it solve.]

</div>

## Overview

Explain the core architecture or main features.
...
- **`SECURITY.md`** — only commit this *after* enabling private vulnerability reporting
  (Phase 4), since it points contributors at the report button:
...
- **`.github/workflows/ci.yml`** — language-detected. Skeleton (fill in real setup and
  commands; if the stack is unknown, skip this file and report it):
...
- **`.github/workflows/release-please.yml`** — only if the user confirmed versioned
  ...
  (replacing both a hand-maintained CHANGELOG and release-drafter). Set `release-type`
...
- **`.github/workflows/docker-publish.yml`** — only if a `Dockerfile` exists:
...
- **Existing code**: create branch `chore/setup-github-repo`, stage the generated
  files by explicit path, commit, `gh pr create`, wait for CI, and merge. Settings and
  rulesets come after the merge.
