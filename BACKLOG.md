...
- **Why deferred:** User chose "cron only, first" — wanted the zero-risk time-based win before adding the request-path interceptor. The HTTP add-on inserts a proxy hop (`Traefik → interceptor → pod`), which means rewiring each app's Gateway API `HTTPRoute` to the interceptor Service plus a per-app scaling CR. A first implementation attempt was reverted on 2026-06-23 after we found the obvious CR (`HTTPScaledObject`) is **deprecated** in add-on v0.15 — the current API is `InterceptorRoute` (v1beta1), which has a materially different model (no `scaleTargetRef`/`replicas` on the CR). Needs more research before rebuilding.
- **Unblock:** Pick up from the detailed handover — **`k8s/talos/infra/keda/PHASE2-HTTP-ADDON-HANDOVER.md`** — which has the confirmed chart facts (0.15.0, interceptor `keda-add-ons-http-interceptor-proxy:8080`), the full `InterceptorRoute` schema, per-app routing table, the planned ReferenceGrant + CNP changes, and the one open question to resolve first (how the workload Deployment is scaled to/from zero under InterceptorRoute — read the official "Migrate from HTTPScaledObject to InterceptorRoute" guide).
- **Where:** new `k8s/talos/infra/keda-http-add-on/`; per-app `k8s/talos/apps/{it-tools,omni-tools,headroom,portfolio-stage,blog-stage}/{httproute.yaml,ciliumnetworkpolicy.yaml}` + new InterceptorRoute CR (replacing the phase-1 cron `scaledobject.yaml`). Full blueprint in the handover doc above.
