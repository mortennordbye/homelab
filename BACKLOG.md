...
- **Note:** authentik's CNP was written in `ab6f6dfc` but never listed in its `kustomization.yaml`, so it was not loaded until 2026-09-16 — the identity namespace has no audit history before that date and its own ≥7d window starts there, separately from the rest. Its egress allows DNS and in-namespace only, and the first two hours of audit already show two gaps, both from `authentik-worker`: egress to `reserved:kube-apiserver` and egress to `reserved:world`. Both need allow rules before `identity` enforces. Keep watching for periodic flows the first window misses.
