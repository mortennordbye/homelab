...
### Cluster backups: steps after etcd
- **What:** the rest of `docs/backup-plan.md` section 5: `pg_dump` CronJobs for logeverylift and Authentik, daily app-native backups in the *arr apps, Plex, Tautulli and Tdarr, VolSync restic for the app volumes, adding `k8s-backups` to the Google Drive Hyper Backup task, and a restore test with a runbook. Also delete logeverylift's unused pre-18 `postgres-pvc`.
- **Why deferred:** rolled out one layer per PR, each validated before the next; Home Assistant and etcd went first.
- **Unblock:** the etcd CronJob producing snapshots on the NAS. VolSync then starts with a one-volume pilot to prove the NFS-path restic repository and mover permissions.
- **Where:** `docs/backup-plan.md`, `k8s/talos/infra/etcd-backup/`, `k8s/talos/infra/kube-prometheus-stack/homelab-alerts.yaml`.
