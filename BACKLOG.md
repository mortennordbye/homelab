...
- **What:** `/fun` is the real apartment with all four spaces walkable and the north wall glazed, but three things on the floor plan are still not built: the **Skap** (closet) off the entré, the **P** marked in the living room (a post, or a flue), and **door leaves** for the bedroom and bathroom — those openings are cased and lined but carry no door, so neither room can be shut.
- **Why deferred:** None of the three changes whether the flat is navigable or whether it reads correctly, and the shell, the windows, the merged TV bench and the furniture were the agreed passes. What the windows look out onto is also still a flat dark plate rather than anything specific; `brand.ts` says what is outside is the cool bounce ("the forest through the window"), so if it ever becomes a view it wants designing rather than filling in.
- **Unblock:** The door leaves can reuse the `Door` component the front door already uses. The Skap is a cupboard box against the bath partition. The post needs confirming against the flat before it is worth modelling.
