...
### KEDA phase 2 — roll the HTTP add-on out to the remaining 4 apps
- **What:** Phase 2 (the KEDA **HTTP add-on**, `keda-add-ons-http`, for daytime wake-from-zero) is now **built and piloting on `portfolio-stage` only**. The add-on infra lives in `k8s/talos/infra/keda-http-add-on/` (chart 0.15.0, interceptor min 2 / max 5, ReferenceGrant scoped to `stage-portfolio`), and `portfolio-stage` swapped its cron `ScaledObject` for an `external-push` one plus an `InterceptorRoute`. Remaining work: replicate the same per-app changes to the other 4 greenlit apps — `it-tools`, `omni-tools`, `headroom`, `blog-stage` — once the pilot is validated. Prod portfolio/blog stay on cron (cold-start latency for real visitors).
- **Why deferred:** User chose "do stage-portfolio only, for now" — validate the request-path interceptor + scale-to-zero on one low-stakes app before fanning out. Each remaining app needs the same four edits: repoint `httproute.yaml` backendRef → `keda-add-ons-http-interceptor-proxy:8080` (keda ns), add the interceptor `fromEndpoints` rule to its CNP on the app's pod targetPort, add an `InterceptorRoute`, and overwrite the cron `scaledobject.yaml` with an `external-push` trigger.
- **Unblock:** Confirm the pilot works (idle `portfolio-stage` scales to 0 after `cooldownPeriod`, then `curl https://portfolio-stage.local.bigd.no` cold-starts it with no 503 lockout). Then for each remaining app follow the per-app facts table in the handover — **`k8s/talos/infra/keda/PHASE2-HTTP-ADDON-HANDOVER.md`** — and add a `from` entry for each app's namespace to `k8s/talos/infra/keda-http-add-on/referencegrant.yaml`.
- **Where:** `k8s/talos/infra/keda-http-add-on/referencegrant.yaml` (add 4 `from` entries); per-app `k8s/talos/apps/{it-tools,omni-tools,headroom,blog-stage}/{httproute.yaml,ciliumnetworkpolicy.yaml,scaledobject.yaml}` + new `interceptorroute.yaml` and `kustomization.yaml` wiring. `portfolio-stage` is the worked reference example.
