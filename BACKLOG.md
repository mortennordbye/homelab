...
- **Where:** `k8s/talos/infra/traefik/values.yaml`, `k8s/talos/apps/{portfolio,blog,reelsmith,headroom-demo,logeverylift}/httproute.yaml`.
...
- **Note (2026-08-21):** `ddns.nordbye.it` is now proxied, so neither `nordbye.it` nor `logeverylift.com` publishes the residential address. `ddns.bigd.no` still does, and `bigd.no` itself resolves straight to it, so the address stays public until this lands. `audiobookshelf` and the plex hostnames cannot be proxied, so they need Tailscale or another private path rather than a proxy annotation, otherwise one unproxied hostname keeps publishing it. The same blocker rules out a WAN firewall rule allowing only Cloudflare ranges.
