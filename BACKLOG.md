...
- **Where:** `k8s/talos/infra/traefik/values.yaml`, `k8s/talos/apps/{portfolio,blog,reelsmith,headroom-demo,logeverylift}/httproute.yaml`. The seven proxied `bigd.no` hostnames belong in the same allowlist, but `audiobookshelf` and `seerr` must stay out of it while they are reached directly.

### The residential address is published in the bigd.no zone
- **What:** `bigd.no`, `www.bigd.no`, `hub.bigd.no`, `auth.bigd.no`, `it-tools.bigd.no`, `mealie.bigd.no` and `omni-tools.bigd.no` are proxied. `audiobookshelf.bigd.no`, `seerr.bigd.no` and `ddns.bigd.no` are not, so the address is public in this zone despite `nordbye.it` and `logeverylift.com` being clean.
- **Why deferred:** audiobookshelf and seerr both serve users outside the household and must stay publicly reachable, so moving them behind the tailnet is ruled out even though the subnet router and the `*.local.bigd.no` private gateway would carry them today. audiobookshelf cannot be proxied either: Cloudflare's terms exclude serving audio through the CDN outside Enterprise, enforced per account rather than per hostname. `ddns.bigd.no` is therefore pinned unproxied too, because Cloudflare resolves a DNS-only CNAME through to its target's answer, so proxying it would route every hostname pointing at it through the edge whatever its own proxy flag says. Verified in the zone with a throwaway CNAME.
- **Unblock:** only seerr is still movable, and it is ready: its `cacheImages` setting reads `false` in `/app/config/settings.json`, so it loads TMDB posters straight from `image.tmdb.org` and nothing third-party flows through the origin. Its HTTPRoute annotation can flip to `"true"` like the others, since external-dns owns the record. That buys seerr the WAF and the edge rate limits but hides nothing, because audiobookshelf and `ddns` keep the address public regardless. Hiding it outright needs either a Cloudflare plan that permits audio or a second CDN in front of audiobookshelf alone, and Plex would still hand the address to plex.tv (see the Plex remote-access entry below).
...
- **What:** The cluster runs `policyAuditMode: true` (`k8s/talos/infra/cilium/values.yaml`), so the 45 CiliumNetworkPolicies log but never drop. The audit window has now elapsed and the query below was run on 2026-09-16 over 7d. It returns 41 distinct flows that would be denied today. Writing the allow rules is what is left.
- **The gaps, by theme:**
  - `monitoring` egress to `reserved:kube-apiserver`: grafana (40k), loki (20k).
  - `monitoring` alertmanager peer gossip, both directions (20k each).
  - Prometheus scraping: `portfolio` (31k) and `portfolio/status-publisher` (1k).
  - Egress to `reserved:world`: `reelsmith-gateway` (28k), `external-secrets/bitwarden-sdk-server` (24k), `argocd-repo-server` (13k), plus low-volume grafana, traefik, alertmanager, alloy, prometheus, external-dns, headroom-demo, gluetun.
  - Ingress from `traefik/traefik`: plex-media-stack (17k), seerr (8k), arr-stack, argocd, homepage, verksted, and the `*-cd` namespaces.
  - `arr-stack/exportarr-bazarr` to `bazarr` (5k).
  - `reserved:world` to `plex-media-stack/plex` (3k), which is Plex remote access.
  - `identity/authentik-worker` to `reserved:kube-apiserver` and to `reserved:world`, and `authentik-server` to world. Its CNP was only loaded on 2026-09-16, so its own history is shallow and needs re-checking after a week.
- **Why deferred:** The rules themselves are the work, and there are roughly 20 namespaces to touch. Getting one wrong takes an app off the network, so this wants a dedicated session rather than a tail end of one.
- **Blocker found 2026-09-16:** the plan of flipping enforcement namespace by namespace does not work as written. `policyAuditMode` is a single agent-wide value in the `cilium-config` ConfigMap, confirmed with `kubectl get configmap -n kube-system cilium-config -o jsonpath='{.data.policy-audit-mode}'`. There is no declarative per-namespace toggle, so setting it to `false` enforces all 45 policies at once. Either every gap above is closed before the flip, or the flip is staged by temporarily removing the CNPs from namespaces that are not ready, which is backwards. Per-endpoint audit can be set imperatively with `cilium-dbg endpoint config <id> PolicyAuditMode=false`, which suits spot-checking a single workload but is not GitOps.
