...
> 🔍 **Click the diagram to open the full-size, zoomable SVG** — it's vector, so it stays sharp at any zoom. The whole homelab at a glance — Internet → UniFi network edge → Proxmox hosts → the Genesis Talos cluster (every platform service and application) → storage, plus the smart-home branch and the GitOps loop. Auto-generated by the [render pipeline](.github/workflows/render-diagram.yaml) from [`docs/diagrams/homelab-overview.d2`](docs/diagrams/homelab-overview.d2); logos from [dashboard-icons](https://github.com/homarr-labs/dashboard-icons) & [selfh.st](https://github.com/selfhst/icons).
...
> 🔍 **Click the diagram to open the full-size, zoomable SVG** — it's a wide, detailed graph, so the inline preview is shrunk to fit; open it to read the labels. Who talks to whom — every service as its own node with the real edges between them: Traefik → each public site, Seerr → Sonarr/Radarr → qBittorrent → Gluetun → VPN, ESO → each secret consumer, Grafana → Prometheus/Loki/Tempo, Falco → Falcosidekick → Discord, Home Assistant → each device, and more. Color-coded by concern: traffic (blue), TLS/DNS (cyan), GitOps (indigo), secrets (purple), media (pink), VPN (orange), observability (teal), security/alerts (red), storage (amber), smart-home (green). Scalable SVG — open it to zoom into the detail. Auto-generated by the same [render pipeline](.github/workflows/render-diagram.yaml) from [`docs/diagrams/network-flow.d2`](docs/diagrams/network-flow.d2).
