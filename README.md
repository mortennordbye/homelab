...
> Auto-generated by a [GitHub Actions pipeline](.github/workflows/render-diagram.yaml): on every change to the declarative source [`docs/diagrams/architecture.d2`](docs/diagrams/architecture.d2), CI re-renders this image with [D2](https://d2lang.com/) and commits it back — so it never drifts from the source. Icons are inlined at render time, so the image is self-contained. (Local preview: `make diagram`.)
