...
outcomes:
  ...
  - "Production cutover ran service by service across roughly 25 windows from late January to 19 February 2026, with one planned downtime window and no service rolled back."
...
I came in as a Cloud engineer on a three-person Orange-side team. The previous architect set the direction; I owned most of the code that changed. When that architect rolled off in April 2026, I took over architect responsibility on the Orange side, and every technical decision on the account went through me from then on.
...
Post-takeover, I drove the post-migration architecture across ServiceBus migration into the customer's new subscription, the move of the customer's main domain onto Azure DNS, and a cold-redeploy DR plan for Azure region failure with runbooks for first-line. Policy-as-code via EPAC did not land inside the engagement window. I wrote it up as an architecture decision record for the customer's CISO instead, so the incoming team has the decision rather than an abandoned half-implementation. Worked alongside the customer's team to stabilise production after migration, where a recurring .NET thread-pool starvation pattern was surfaced via observability, reproduced in a replication harness built from the customer's components, and resolved through an async refactor led by the customer's developers.
...
A clean exit from Orange-hosted Windows and .NET onto a cloud-native Azure architecture the customer can grow into. The engagement closed in August 2026 with the platform handed to Managed Services and the customer's technical account manager, who run it without me.
