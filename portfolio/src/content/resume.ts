...
const experienceRaw: Experience[] = [
  {
    ...
    description: [
      ...
      "Drove the post-migration architecture, including the ServiceBus migration into the customer's new subscription, the move of the customer's main domain onto Azure DNS, and a cold-redeploy DR plan for Azure region failure with runbooks for first-line. Policy-as-code via EPAC did not land inside the engagement window, so it went to the customer's CISO as an architecture decision record instead. The engagement closed in August 2026 with the platform handed to Managed Services and the customer's technical account manager. Also worked alongside the customer's team to stabilise production after migration, where a recurring .NET thread-pool starvation pattern was surfaced via observability, reproduced in a replication harness built from the customer's components, and resolved through an async refactor led by the customer's developers.",
      ...
      "For the internal Orange department, designed and delivered the Azure platform under their Entra ID and Microsoft 365 automation products, which govern configuration as code across 15 customer tenants. Covered the WAF Application Landing Zone subscription layout, ACR and Container Apps for hosting, Azure DNS, and the GitHub repository with Terraform pipelines so the receiving team runs it themselves, with image deploys wired so Terraform stays the source of truth for what runs and federated OIDC replaces long-lived tokens.",
      "On the public-sector engagement, audited an Azure estate delivered by lift-and-shift and turned the findings into measures the operating team owns. Traced 90 days of SQL point-in-time-restore gaps to their triggers, built an Azure workbook that makes each broken backup chain and its cause visible, and closed a 250-day patch gap on the Linux hosts with a routine for keeping them current.",
